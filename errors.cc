...
void ReportError::ImplementIsNotInterface( Decl *cd,Type *intfType )
{
	ostringstream s;
	s << "Class '" << cd << "' has implemented a Not Interface '" << intfType << "'";
	OutputError(intfType->GetLocation(), s.str());
}
...
void ReportError::NewArrayTypeWrong( Type* type )
{
	ostringstream str;
	str<<"New Array operate on "<<type->typeName<<endl;

	OutputError(type->GetLocation(),str.str());
}
...
void ReportError::ReturnOutFunction(ReturnStmt* Ret) {
		OutputError(Ret->GetLocation(), "return is only allowed inside a function");
}

void ReportError::MainFunctionHadNotDeclare()
{
	OutputError(NULL,"Linker error: function 'main' not defined");
}

void ReportError::MainFunctionIsCompatibleToStandard( Decl *fnDecl )
{
	ostringstream s;
	s << "Method '" << fnDecl << "' must be No Formals and Return type is int)";
	OutputError(fnDecl->GetLocation(), s.str());
}

void ReportError::NoMainFound()
{
	MainFunctionHadNotDeclare();
}
