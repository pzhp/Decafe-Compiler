...
Location::Location(Segment s, int o, const char *name) :
  variableName(_strdup(name)), segment(s), offset(o){}
...
LoadStringConstant::LoadStringConstant(Location *d, const char *s)
  ...
  sprintf(str, "%s%s%s", quote, s, quote); //{ "abc" } 
  quote = (strlen(str) > 50) ? "...\"" : "";
...
Label::Label(const char *l) : label(_strdup(l)) {
  Assert(label != NULL);
  *printed = '\0';
}
void Label::Print() {
...
Goto::Goto(const char *l) : label(_strdup(l)) {
  Assert(label != NULL);
...
VTable::VTable(const char *l, List<const char *> *m)
  : methodLabels(m), label(_strdup(l)) {
