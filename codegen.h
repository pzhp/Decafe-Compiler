...
public:
	...
	void PrintAllInstrutions()
	{
		for (int i=0,n=code->NumElements();i<n;i++)
		{
			code->Nth(i)->Print();
		}		
	}
  ...
  public:
    ...
    char *NewLabel();
	     ...
	     // 只是一个占位作用
    Location *GenTempVar(); //for {int x;} forms ,use for link varible and location
    Location *GenLocalVar(const char *name, int size); 
	Location * GenGlobalVar(const char* name,int size);
    ...
    Location *GenLoadConstant(int value);       //li reg 12 
    Location *GenLoadConstant(const char *str); // 先将字符存到data域，再la ** str_label
    Location *GenLoadLabel(const char *label);  //la reg label
    ...
    void GenLabel(const char *label);  //use for function label or loop label
